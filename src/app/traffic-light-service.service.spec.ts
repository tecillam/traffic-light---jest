...
import {
  ...
  TrafficLight,
  TrafficLightService,
} from './traffic-light-service.service';
...
describe('TrafficLightService', () => {
  ...
  beforeEach(() => {
    ...
    trafficLightService = TestBed.inject(TrafficLightService);
  });

  // skip test automatically generated
  xit('should be created', () => {
    expect(trafficLightService).toBeTruthy();
  });

  it('Test complete simulation', () => {
    const result = trafficLightService.simulateRoadTraffic('C..CO....CG..', 6);
  ...
  it('This should parse the initial road correctly', () => {
    const result = trafficLightService.parseInitialRoad('.C.C..R.C..G...');
  ...
  it('This call should fail due to invalide characters on input road', () => {
    expect(() =>
      trafficLightService.parseInitialRoad('..X2.Y..T.')
    ).toThrowError('Invalid character in road string');
  });

  // Negative test over simulateRoadTraffic
  it('This call should fail due to empty road', () => {
    expect(() => trafficLightService.simulateRoadTraffic('', 3)).toThrowError(
  ...
  it('This call should fail due to negative iterations', () => {
    expect(() =>
      trafficLightService.simulateRoadTraffic('..C.O..', -3)
    ).toThrowError('Iterations cannot be negative');
  });
  ...
  it('should transition traffic light from G to O when timer = 5', () => {
    ...
    // test G => O
    trafficLightService.updateLights(light);
  ...
  it('Both cars should move', () => {
    ...
    const carsUpdated = trafficLightService.updateCarPosition(
      cars,
      road,
      light
    );
  ...
  it('This should test final output array build', () => {
    const outputArray = trafficLightService.buildRoadString(
      10,
      [{ position: 3 }],
      [{ position: 2, state: 'R', timer: 2 }]
    );
  ...
  it('This test should throw an error due to trafficlight out of range', () => {
    expect(() =>
      trafficLightService.buildRoadString(
        10,
        [{ position: 3 }],
        [{ position: 13, state: 'R', timer: 2 }]
      )
